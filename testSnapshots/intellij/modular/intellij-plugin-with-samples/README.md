...
│       └── resources/      plugin.with.samples.backend.xml
├── frontend/               Frontend module – UI and presentation
│   ├── build.gradle.kts
│   └── src/main/
│       ├── kotlin/         Kotlin production sources
│       └── resources/      plugin.with.samples.frontend.xml
├── shared/                 Shared module – cross-boundary contracts
│   ├── build.gradle.kts
│   └── src/main/
│       ├── kotlin/         Kotlin production sources
│       └── resources/      plugin.with.samples.shared.xml
├── gradle/
...
- `plugin.with.samples.backend.xml` – registers backend extensions and services
- `plugin.with.samples.frontend.xml` – registers frontend extensions and tool windows
- `plugin.with.samples.shared.xml` – registers shared extensions and interfaces
